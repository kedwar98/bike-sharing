...
print ("RF Testing Normalized RMSE: ",mean_squared_error(yLabelstest, testp, squared=False)/(max(yLabelstest)-min(yLabelstest)))
...
print ("GB Testing RMSE: ",mean_squared_error(yLabels, preds, squared=False)

print ("GB Testing Normalized RMSE: ",mean_squared_error(yLabels, preds, squared=False)/(max(yLabels)-min(yLabels)))

testp=gbModel.predict(X=testdata)
#Change testdatawo to testdatawi to include census data

yLabelstest = np.log1p(testdataall['ridesperhour'])
...
model.compile(optimizer='adam',loss=tf.keras.losses.RootMeanSquaredError())
